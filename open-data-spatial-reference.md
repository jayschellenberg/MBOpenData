...
Reusable for any Socrata-hosted or ArcGIS-hosted open data with spatial geometry.
...
Query endpoint: append `/query` to the layer URL and pass parameters.

### Key query parameters

| Parameter | Purpose | Example |
|-----------|---------|---------|
| `where` | SQL WHERE filter (required, `1=1` for all) | `where=1=1` |
| `outFields` | Comma-separated field list (`*` for all) | `outFields=*` |
| `f` | Response format | `f=geojson` or `f=json` |
| `resultOffset` | Skip N rows for pagination | `resultOffset=2000` |
| `resultRecordCount` | Max rows per request | `resultRecordCount=2000` |
| `orderByFields` | Sort order (**critical for consistent pagination**) | `orderByFields=OBJECTID` |
| `outSR` | Output spatial reference (WKID) | `outSR=4326` |
| `returnCountOnly` | Return only the count | `returnCountOnly=true` |

### Get total row count first

```r
base_url <- "https://services.arcgis.com/{orgId}/arcgis/rest/services/{service}/FeatureServer/{layer}"
...
```r
batch_size <- 2000  # Check maxRecordCount in service metadata (?f=json)
n_batches <- ceiling(total_rows / batch_size)
offsets <- seq(0, by = batch_size, length.out = n_batches)
```

**Critical**: Always include `orderByFields=OBJECTID` — without it, the API does not
guarantee consistent row ordering across paginated requests.

An empty `features` array in the response signals the end of data. The response may
also include `exceededTransferLimit: true` to confirm more pages exist.

### GeoJSON vs JSON vs CSV tradeoffs (ArcGIS)

| Format | Geometry included | CRS handling | Parse method |
|--------|-------------------|--------------|-------------|
| `f=geojson` | Yes, auto-reprojected to WGS84 | Automatic (always 4326) | `st_read()` on response body |
| `f=json` (EsriJSON) | Yes, in native CRS | Must use `outSR=4326` or transform locally | Requires esri2sf or manual parsing |
| `f=csv` | No | N/A | `read_csv()` but need separate geometry fetch |

**Recommendation**: Use `f=geojson` for datasets with geometry. `st_read()` parses it
directly into sf objects with correct CRS. No WKT conversion or CRS transform needed.

### Batch downloader template (ArcGIS)
...
download_batch <- function(base_url, batch_size, offset, batch_num, n_batches) {
  ...
  cli_alert_success(
    "Batch {batch_num}: {format(nrow(batch_sf), big.mark = ',')} rows"
  )
  batch_sf
}
```

**Note**: Each batch returns an sf object directly — no WKT-to-sf conversion needed
(unlike the Socrata CSV pipeline). Combine batches with `bind_rows()`.
...
### Issue: ArcGIS `f=geojson` auto-reprojects to WGS84
**Symptom**: Coordinates are in lon/lat even though native CRS is UTM.
**Explanation**: The `f=geojson` format always returns EPSG:4326. This is expected behaviour — do NOT apply `st_transform()` after reading.

### Issue: ArcGIS `Total_Value` is a formatted string
**Symptom**: `Total_Value` contains `"$359,500"` instead of a number.
**Fix**: `mutate(Total_Value_Num = readr::parse_number(Total_Value))` before writing GeoParquet.

### Issue: ArcGIS `maxRecordCount` varies by service
**Symptom**: More batches needed than expected, or fewer features returned than `resultRecordCount`.
**Fix**: Always check the service metadata at `{endpoint}?f=json` for `maxRecordCount` before designing pagination. Common values: 1,000 or 2,000.
...
## 10. Manitoba Project Specifics (for reference)

- **Portal**: Manitoba GeoPortal / ArcGIS Hub (`geoportal.gov.mb.ca`)
- **Service**: ROLL_ENTRY (MG_ROLL_ENTRY_POLY)
- **API base**: `https://services.arcgis.com/mMUesHYPkXjaFGfS/arcgis/rest/services/ROLL_ENTRY/FeatureServer/0`
- **Rows**: ~437,184 (as of 2026-03-10)
- **Columns**: 13 fields + geometry (OBJECTID, Roll_No, Roll_No_Txt, Property_Address, Municipality, Muni_Name_With_Typ, Asmt_Roll, Dwelling_Units, Frontage_or_Area, Total_Value, Asmt_Rpt_Url, Shape__Area, Shape__Length)
- **Derived columns**: `Total_Value_Num` (numeric, parsed from `Total_Value` string)
- **Native CRS**: EPSG:26914 (UTM Zone 14N) — auto-reprojected to 4326 via `f=geojson`
- **Max records per query**: 2,000 (with geometry)
- **Key columns**: `Roll_No_Txt` (string ID), `Total_Value` (formatted string), `Total_Value_Num` (derived numeric), `Muni_Name_With_Typ`, `Property_Address`, `Dwelling_Units`
- **GeoParquet size**: ~60-80 MB (estimated, 437k polygon features)
- **PMTiles size**: ~80-120 MB (estimated, zoom 5-14)
- **Map center**: `c(-98.0, 55.0)`, zoom 5
